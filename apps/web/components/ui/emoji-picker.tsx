...
function EmojiPicker({
  ...
  return (
    <EmojiPickerPrimitive.Root
      className={cn(
        "bg-popover text-popover-foreground isolate flex size-full flex-col overflow-hidden rounded-md",
        className
      )}
  ...
  return (
    ...
    >
      ...
      <EmojiPickerPrimitive.Search
        className="placeholder:text-muted-foreground flex h-10 w-full rounded-md bg-transparent py-3 text-sm focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
...
function EmojiPickerContent({
  ...
  return (
    <EmojiPickerPrimitive.Viewport
      className={cn("relative ml-4 w-fit flex-1 overflow-hidden pr-5", className)}
  ...
  return (
    ...
    >
      <EmojiPickerPrimitive.ActiveEmoji>
        {({ emoji }: { emoji?: { emoji: string; label: string } }) =>
          emoji ? (
            <>
